class UrlLinksController < ApplicationController
  ...
  def show
    ...
    add_breadcrumb @url_link.link.name, @url_link 
  end
  ...
  def update
    respond_to do |format|
      if @url_link.update(url_link_params)
        format.html { redirect_to link_url(@url_link), success: "Link was successfully updated." }
  ...
  private
    ...
    def url_link_params
      params.require(:url_link).permit(:url, links_attributes: [:user_id, :name])
    end
end
