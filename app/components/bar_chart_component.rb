...
class BarChartComponent < ViewComponent::Base
  ...
  def bar(title, count)
    content_tag(:div, class: "w-full relative", style: "max-width: calc(100% - 5rem);") do
      content_tag(
        :div, nil,
        class: "absolute top-0 left-0 h-full bg-gray-200 dark:bg-gray-600",
  ...
  def bar_text(title)
    content_tag(:span, class: "flex px-2 py-1.5 group relative z-9 break-all") do
      if title.present?
        link_to(
          request.params.merge("#{@label_unit.downcase}_filter": title),
          class: "overflow-hidden hover:underline decoration-gray-800 dark:decoration-gray-100",
          data: { turbo_action: "advance", turbo_frame: "scans" }
        ) do
          content_tag(:p, class: "truncate block text-gray-800 dark:text-gray-100") do
            title
          end
        end
      else
        content_tag(:p, "Unknown", class: "block text-gray-800 dark:text-gray-100")
      end
    end
  end
end
